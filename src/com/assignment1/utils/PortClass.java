...
import com.Server.implementationV.LibraryServer;
		...
		try {
			...
			while (serverInstance.isStopServer()) {
				...
				if (!listenForInterLibraryActions && datArry.length == 3) {
					try {
						response = serverInstance.getNonRetuners(datArry[0],
								datArry[1], name, Integer.parseInt(datArry[2]));
					} catch (Exception e) {
						response = "Exception Occurred : " + e.getMessage();
						e.printStackTrace();
					}
				} else if (listenForInterLibraryActions && datArry.length == 3) {
					try{
						serverInstance.reserveBookCore(datArry[0], datArry[1], datArry[2]);
						String logFile = Configuration.CURRENT_DIR + name + "//"
								+ datArry[2]+".txt";
						FileOps.appendToFile(logFile, new StringBuilder(datArry[2]
								+ ": reserveBook :Book : " + datArry[0]
								+ " by Author : " + datArry[1]
								+ " has been reserved for Inter library Transfer By "+datArry[2]+" library.."));
						response = "1";
					}
					catch(Exception e){
						response = "0";
